package everything.inzent.ecm.file;
...
public class MyFile {
	...
	public String[][] getList(String address) {
		...
		/*
		 * 1. 검색할 디렉토리 명을 받아온다. 2. 디렉토리 명이 입력되지 않았으면 루트 디렉토리부터 전체 검색 한다. 3. listFile()로
		 * 해당 디렉토리 내 모든 파일의 절대경로를 알아낸다. -> fileList 4. 알아낸 절대경로를 통해 파일명만 따로 뽑아낸다. 5. new
		 * File(filePath) -> file.length(); 로 파일 크기 구한다. 6. 파일 생성일자를 구한다. 7. 3,4,5,6 을
		 * 통해 알아낸 이차원 배열에 넣어준다(Swing 테이블에 값 입력해주기)
		 */
	...
	private void getFileName(List<String> fileList, List<String> fileNameList) {
		...
		/*
		 * 1. 디렉토리 내 모든 파일들의 전체 경로를 추출해서 filePathList에 담는다 2. filePathList 값들을 \ 기준으로
		 * 나눠서 배열에 담는다. 3. 배열을 list 로 변환한다. 4. 변환한 list 를 reverse() 해서 파일명이 0번째 인덱스가
		 * 되게한다. 5. 0번째 인덱스만 뽑아서 fileNameList에 담는다.
		 */
		System.out.println("=======================================");
