...
public class Frame extends JFrame {
	...
	/*
	 * 1. new Frame() 하면 상단 패널만 존재하는 frame 생성하기 2. 상단 패널에 디렉토리 입력 텍스트필드 추가 3. 검색버튼추가
	 * 4. 검색버튼 추가시 하단 패널 띄우기 4-1. 하단 패널 띄울 때 2번의 텍스트필드 값 매개변수로 넘기기 5. 매개변수로 받은 값파일
	 * 불러오는 매서드 매개변수로 넘기기
	 */
...
//		frame = new JFrame("EveryThing_By_Liz");
		super.setSize(1027, 689);

		JPanel panelTop = new JPanel();
		getContentPane().add(panelTop);
		panelTop.setBounds(6, 10, 850, 50);
		...
		bottomPanel.setBounds(6, 70, 850, 500);
		...
		sl_panel.putConstraint(SpringLayout.WEST, textField_2, 39, SpringLayout.WEST, bottomPanel);
		bottomPanel.add(textField_2);

		JButton btn1_1 = new JButton("검색");
		sl_panel.putConstraint(SpringLayout.EAST, textField_2, -6, SpringLayout.WEST, btn1_1);
		sl_panel.putConstraint(SpringLayout.WEST, btn1_1, 768, SpringLayout.WEST, bottomPanel);
		sl_panel.putConstraint(SpringLayout.NORTH, btn1_1, 65, SpringLayout.NORTH, bottomPanel);
		sl_panel.putConstraint(SpringLayout.NORTH, textField_2, 1, SpringLayout.NORTH, btn1_1);
		bottomPanel.add(btn1_1);

		tablePanel = new JPanel();
		sl_panel.putConstraint(SpringLayout.NORTH, tablePanel, 112, SpringLayout.NORTH, bottomPanel);
		sl_panel.putConstraint(SpringLayout.EAST, tablePanel, -34, SpringLayout.EAST, bottomPanel);
		sl_panel.putConstraint(SpringLayout.WEST, tablePanel, 39, SpringLayout.WEST, bottomPanel);
		sl_panel.putConstraint(SpringLayout.SOUTH, tablePanel, -10, SpringLayout.SOUTH, bottomPanel);
